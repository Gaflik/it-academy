...
type Person = AdminInterface | UserInterface;
 ...
 */
export function map(mapper: (firstNum: number, secondNum: number) => number, input: number[]): number[] | Function {
 ...
 * @return {Array | Function}
 */
export function filter(filterer: (firstValue: string, secondValue: number, thirdValue: string[]) => string, input: string[]): string[] | Function  {
    if (arguments.length === 0) {
        return filter;
    }
    if (arguments.length === 1) {
